...
const   loginUser = async (req, res) => {
  try {
    ...
    if (!isMatch) {
      ...
      const user = await employees.findById({
        _id: decoded._id,
        "tokens.token": token,
      });
...
const allUsersByHod = async (req, res) => {
  try {
    ...
    const users = await employees.find({
      $and: [{ roles: "STAFF" }, { position: hodPos }],
    });
    if (!users) {
      res.json({ error: "No users are there" });
    } else {
...
const verifyPassOtp = async function (req, res) {
  try {
    ...
    const isExpire = await otpModel.findOne({
      $and: [{ email: email}, {otp: otp}, {otpExpires: { $gt: Date.now() } }, {otpUsed : 0}],
    });
    if (!isExpire) {
...
const resetPassword = async function (req, res) {
  try {
    ...
    if (cPassword !== password) {
      res.json({ error: "Password and Confirm Password does not match" });
    }
    ...
    const isExpire = await otpModel.findOne({$and: [
        { email: email, otp: otp, otpExpires: { $gt: thirtyMinutesFromNow }, otpUsed : 0 },
      ]});
    if(isExpire){
        await otpModel.deleteMany({ email: email });
        res.json({ error: "Reset TimeOut !!" });
    }
